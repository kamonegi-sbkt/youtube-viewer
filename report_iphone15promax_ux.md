...
| | 内容 | 本番検証 |
|---|---|---|
| **E** | **履歴からの個別削除**（追加のみで削除導線がなかったギャップを解消）。履歴カードに「履歴から削除」×を追加。トースト/undoを汎用化し新着の「非表示」と復元UIを共有 | ✅ 削除→即undoで `historyLS` 復元(1/1)を本番確認 |
| **F** | **ローディングのスケルトン化**（スピナー→16:9プレースホルダ＋shimmer＋2行テキスト×6） | ✅ feed遅延時にスケルトン6枚＋aria-busy表示を本番で視覚確認 |

> 検証メモ: undoは当初「効かない」と見えたが、**CLI操作のレイテンシがトーストの5秒自動消滅を超えていた**ための空クリックが原因と判明。待機なしの連続操作で feed/history とも完全復元を確認（バグではなかった）。

## 7. 残課題 / 今後の提案（未実装）

- **pull-to-refresh の物理スワイプ最終確認**: 実機 iPhone で1回タッチ確認推奨（ロジック・配線は投入済み、ヘッドレスWebKitは `new Touch` 不可で自動検証不能）。
- **「あとで」からの個別削除の明示化**: 現状はブックマーク解除で対応（機能上は可能）。
